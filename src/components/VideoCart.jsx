...
const VideoCart = ({ props }) => {
    ...
    const getChannelIcon = async () => {
        try {
            const channelData = await axios.get(`${import.meta.env.VITE_YOUTUBE_VIDEO_BY_ID}${props.snippet.channelId}&key=${import.meta.env.VITE_YOUTUBE_VIDEO_API_KEY}`); // fetch each channels details.
            // console.log(res)
    ...
    return (
        <div className={`w-full mb-8 shadow-black/100 shadow-lg rounded-lg cursor-pointer opacity-100 hover:opacity-50 animated-border hover:border transition-opacity duration-250`}>
            ...
            <div className='flex gap-3 p-2 font-bold'>
                <div><Avatar src={channelIcon} size="37" className='rounded-full' />
                </div>
